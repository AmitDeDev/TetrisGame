...
public class GridManager : MonoBehaviour
{
    ...
    private static void TriggerRowClearEffect(int y)
    {
        ...
        {
            foreach (var prefab in Instance.colorParticles)
            {
        ...
        else
        {
            ...
            Sprite target = rowOwnerSprites[y];
            int index = Instance.referenceSprites.IndexOf(target);
            if (index >= 0 && index < Instance.colorParticles.Count)
            {
                var prefab = Instance.colorParticles[index];
                if (prefab != null)
                {
                    var fx = Instantiate(prefab, position, Quaternion.identity);
                    var ps = fx.GetComponent<ParticleSystem>();
    ...
    public static void ShiftRowsUp()
    {
        var spawnPoint = Mathf.RoundToInt(FindObjectOfType<ShapeSpawnerController>().spawnPosition.y -1);

        DestroyRowAtTheTop(spawnPoint);
        
        // move every other block up
        for (int y = spawnPoint - 1; y >= 0; y--)
        {
            for (int x = 0; x < GridWidth; x++)
            {
                if (grid[x, y] != null)
    ...
    private static void DestroyRowAtTheTop(int spawnPoint)
    {
        for (int x = 0; x < GridWidth; x++)
        {
            if (grid[x, spawnPoint] != null)
            {
                Destroy(grid[x, spawnPoint].gameObject);
                grid[x, spawnPoint] = null;
                FindObjectOfType<ShapeSpawnerController>().enableRisingLines = false;
